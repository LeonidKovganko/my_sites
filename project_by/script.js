...
function Sim2(sldrId) {
	...
	this.leftArrow = this.sldrRoot.querySelector('div.sim-slider-arrow-left1');
	this.rightArrow = this.sldrRoot.querySelector('div.sim-slider-arrow-right1');
	this.indicatorDots = this.sldrRoot.querySelector('div.sim-slider-dots1');
...
Sim2.initialize = function (that) {
	...
	if (that.options.dots) {  // инициализация индикаторных точек
		let sum = '', diffNum;
		for (let i = 0; i < that.elemCount; i++) {
			sum += '<span class="sim-dot1"></span>'
		};
		that.indicatorDots.innerHTML = sum;
		that.indicatorDotsAll = that.sldrRoot.querySelectorAll('span.sim-dot1');
		// Назначаем точкам обработчик события 'click'
		for (let n = 0; n < that.elemCount; n++) {
			that.indicatorDotsAll[n].addEventListener('click', function () {
				diffNum = Math.abs(n - that.currentElement);
...
Sim3.prototype.dotOn = function (num) {
	this.indicatorDotsAll[num].style.cssText = 'background-color:#BBB; cursor:pointer;'
};

Sim3.prototype.dotOff = function (num) {
	this.indicatorDotsAll[num].style.cssText = 'background-color:#556; cursor:default;'
};
